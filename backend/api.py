...
**Task**: Analyze the provided resume(s) to extract and present relevant candidate information in a clear, structured format.
...
2. For skill-specific queries (e.g., "React developers"):
   - Focus on relevant technical skills
   - Highlight years of experience with the technology
   - Note any significant projects using the technology

3. For experience-based queries:
   - Extract years of relevant experience
   - List key roles and responsibilities
   - Highlight industry-specific experience

4. For role-specific queries:
   - Match candidate qualifications against role requirements
   - Highlight relevant experience and skills
   - Note any certifications or specialized training

**Output Format**:

For individual candidates:
Candidate: [Full Name]
- Technical Skills: [List of relevant skills]
- Experience: [Years] years
- Current Role: [Position] at [Company]
- Key Projects: [Brief description of relevant projects]
- Education: [Degree and Institution]

For multiple candidates:
[Repeat the above format for each matching candidate]

**Important Guidelines**:
- Only include information explicitly stated in the resume
- Focus on information relevant to the query
- Present information in a clear, bullet-point format
- If information is not found, state "Information not available"
- Do not make assumptions or include unverified information

**Response**:
[Provide a structured response following the above format]
"""
        ...
        for j in range(i + 1, len(model_names)):
            ...
            if "Error" not in results[model_a] and "Error" not in results[model_b]:
                try:
                    ...
                    model_scores[model_a].append((model_b, rouge_l))
                    model_scores[model_b].append((model_a, rouge_l))
